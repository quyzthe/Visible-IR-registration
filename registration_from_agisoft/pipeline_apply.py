...
<<<<<<< HEAD
    if intr_t is not None:
        ...
        hfov = cfg["agisoft"].get("thermal_hfov_deg")
        if hfov is None:
            print("[WARN] No usable thermal EXIF focal tags, and agisoft.thermal_hfov_deg not set -- "
                  "rig derivation skipped, using 2D for everyone. Set thermal_hfov_deg from your "
                  "camera's published spec sheet (horizontal field of view, degrees) to enable it.")
            return {}
        from PIL import Image
        with Image.open(sample_tpath) as im:
            native_wh_t = im.size
        K_thermal_native = pag.K_from_hfov(hfov, *native_wh_t)
